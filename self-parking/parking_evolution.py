...
pygame.display.set_caption("小车停车入库-backup")
...
def draw_ui(reset_timer_seconds=None):
    ...
    if reset_timer_seconds is not None:
        ...
        timer_surf = font.render(timer_text, True, RED)
...
def simulate_car(genome):
    ...
    player_car_corner = simulate(genome, decoder, parking_rects, obstacle_cars, player_car)
    return player_car_corner
...
def my_fitness_func(genome):
    ...
    fitness = GeneticAlgorithm.car_fitness_from_loss(loss, has_collision, is_loop, is_timeout)
    print(
        f'index:{counter}, loss:{loss}, fitness:{fitness}, collision:{collision_count} wheels:{wheels}, gene:{genome}')
    counter = counter + 1
    return fitness
...
def starter():
    ...
    ga = GeneticAlgorithm(
        r'C:\Users\hi\Downloads\genetic_test.csv', #日志路径
        genome_length=180,
        population_size=100,
        mutation_rate=0.05,
        elite_ratio=0.2,
